...
// Criar um roadmap para um aluno pela tela do admin
router.post("/roadmap", auth, async (req, res) => {
  try {
    const { roadmap_id, user_id } = req.body;

    // Verificar se o roadmap existe
    const roadmap = await Roadmap.findById(roadmap_id);
    if (!roadmap) {
      return res.status(404).json({ message: "Roadmap nao encontrado" });
    }
